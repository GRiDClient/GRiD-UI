package com.codingforcookies.mayaui.src.ui.font;
...
public class MayaFontRenderer {
	...
	private static String SAVED_FONT = "";
	public static void pushFont(String name) {
		if(ttfs.containsKey(name)) {
			SAVED_FONT = custFont;
			custFont = name;
		}
	}
	...
	/**
	 * The loaded ttf fonts
	 */
	public static HashMap<String, TrueTypeFont> ttfs = new HashMap<String, TrueTypeFont>();
	public static void registerFont(String name, TrueTypeFont font) {
		ttfs.put(name, font);
	}
	
	private static String custFont = "";
	 ...
	 */
	public static void draw(String string, float x, float y, float size) {
		if(!custFont.equals("")) {
			ttfs.get(custFont).drawString(x, y, string, size, size);
			return;
		}
